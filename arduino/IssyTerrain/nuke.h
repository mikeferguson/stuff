...
#ifdef USE_PC
  #define MOVING   ((Xspeed > 0 || Xspeed < -0) || (Yspeed > 0 || Yspeed < 0) || (Rspeed > 0.0 || Rspeed < -0.0))
  #define SPEED_MULTIPLIER 10
#else
  #define MOVING   ((Xspeed > 10 || Xspeed < -10) || (Yspeed > 10 || Yspeed < -10) || (Rspeed > 0.1 || Rspeed < -0.1))
...
#define Y_COXA      54  // MM between front/back legs /2

/* Legs */
#define L_COXA      52  // MM distance from coxa servo to femur servo
#define L_FEMUR     58 // MM distance from femur servo to tibia servo
#ifdef TASK_MECH
...
#define AMBLE                   5
#define SMOOTH_AMBLE            6

#define TERRAIN_RIPPLE          7
#define TEST_RIPPLE             8
